...
#Verilen sayının 10 tabanında Narsisistik bir sayı olup olmadığına bağlı olarak kodunuz doğru veya yanlış ('doğru' ve 'yanlış' ) döndürmelidir.
def narsisistik(sayi):
    basamaklar = str(sayi)
    basamaksayisi = len(basamaklar)

    # Basamakların kuvvetlerinin toplamını hesaplıyoruz
    toplam = 0
    for basamak in basamaklar:
        toplam += int(basamak) ** basamaksayisi
...
# Test
print(narsisistik(153))  # doğru
print(narsisistik(1652))  # yanlış
